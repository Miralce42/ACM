...
void find_min(int root)
{
	sum += root;
	int l_node = l[root],r_node = r[root];
	if (!l_node && !r_node)
	{
		if (sum < min_sum || (sum == min_sum && root < min_node))
		{
			min_sum = sum;
			min_node = root;
		}
	}
	if(l_node) find_min(l_node);
	if(r_node) find_min(r_node);
	sum -= root;
}
